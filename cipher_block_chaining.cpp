...
void triple_des_encrypt_cbc(const uint8_t *data, uint8_t *encrypted_data, size_t data_len, const uint8_t *key, uint8_t *iv);
...
void triple_des_encrypt_cbc(const uint8_t *data, uint8_t *encrypted_data, size_t data_len, const uint8_t *key, uint8_t *iv) {
    ...
    for (i = 0; i < data_len; i += BLOCK_SIZE) {
        ...
        // Update IV
        memcpy(iv, encrypted_data + i, BLOCK_SIZE);
    }
}

int main() {
    // Sample input data
    const char *sample_data = "This is a test data for 3DES CBC encryption.";
